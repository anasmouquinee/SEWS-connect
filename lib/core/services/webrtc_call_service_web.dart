...
class WebRTCCallServiceWeb {
  ...
  static Future<String> startCall({
    ...
    try {
      ...
      debugPrint('🔄 Getting user media for start...');
      final userMediaResult = _webRTCInstance!.callMethod('getUserMedia', [audioOnly]);
      if (userMediaResult == null) {
        throw Exception('getUserMedia returned null for start');
      }
      await userMediaResult;
      debugPrint('✅ Got user media for start');
      
      // Create peer connection
      debugPrint('🔄 Creating peer connection for start...');
      final peerConnectionResult = _webRTCInstance!.callMethod('createPeerConnection');
      if (peerConnectionResult == null) {
        throw Exception('createPeerConnection returned null for start');
      }
      debugPrint('✅ Created peer connection for start');
      
      // Create offer
      debugPrint('🔄 Creating offer...');
      final offerResult = _webRTCInstance!.callMethod('createOffer');
      if (offerResult == null) {
        throw Exception('createOffer returned null');
      }
      final offer = await offerResult;
      if (offer == null) {
        throw Exception('offer is null after await');
      }
      debugPrint('✅ Created offer: ${offer.runtimeType}');
  ...
  static Future<void> joinCall({
    ...
    try {
      ...
      if (callData == null || callData['offer'] == null) {
        // If no WebRTC document exists, the caller might be using the old system
        // In this case, the joiner should create the call
        debugPrint('⚠️ No WebRTC offer found, joiner will create the call instead');
        
        // Create offer as the joiner (reverse roles)
        final offer = await _webRTCInstance!.callMethod('createOffer');
        debugPrint('✅ Created offer as joiner');
        
        // Save offer to Firebase (create the document)
        await FirebaseFirestore.instance
            .collection('webrtc_calls')
            .doc(channelId)
            .set({
        ...
        debugPrint('✅ Created WebRTC document as joiner');
        
        // Listen for answer and ICE candidates
        _setupSignalingListener(channelId);
        
        debugPrint('✅ Joined call as offerer: $channelId');
        return;
      }
      ...
      // Create answer
      debugPrint('🔄 Creating answer with offer: $offer');
      final answerResult = _webRTCInstance!.callMethod('createAnswer', [
        js.JsObject.jsify(offer)
      ]);
      
      if (answerResult == null) {
        throw Exception('createAnswer returned null');
      }
      
      final answer = await answerResult;
      if (answer == null) {
        throw Exception('answer is null after await');
      }
      
      debugPrint('✅ Created answer: ${answer.runtimeType}');

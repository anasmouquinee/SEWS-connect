...
class _MeetingPageState extends State<MeetingPage> with TickerProviderStateMixin {
  ...
  void _joinMeeting() async {
    ...
    try {
      ...
      // Join the meeting (temporarily disabled for Firebase-only build)
      // await CallService.joinMeeting(meetingId);
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Joined meeting: $meetingId')),
      );
    } catch (e) {
      // Close loading dialog
      if (mounted) Navigator.of(context).pop();
  ...
  void _joinSpecificMeeting(Map<String, dynamic> meeting) async {
    try {
      ...
      // Get room ID from meeting data
      String roomId = meeting['roomId'] ?? meeting['meetingId'] ?? 'default-room';
      
      // Join the meeting (temporarily disabled for Firebase-only build)
      // await CallService.joinMeeting(roomId);
      debugPrint('📞 Joining meeting: $roomId');

      // Close loading dialog
      if (mounted) Navigator.of(context).pop();
      
      // Update meeting status in Firebase
      await FirebaseService.joinMeeting(meeting['id']);
  ...
  void _showMeetingDetails(Map<String, dynamic> meeting) {
    final roomId = meeting['roomId'] ?? meeting['meetingId'] ?? 'unknown';
    final meetingUrl = 'https://meet.sewsconnect.com/$roomId'; // Simplified URL
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            SelectableText(jitsiRoomId),
            const SizedBox(height: 12),
            const Text('Meeting URL:', style: TextStyle(fontWeight: FontWeight.bold)),
            SelectableText(meetingUrl, style: const TextStyle(color: Colors.blue)),
            const SizedBox(height: 12),
        ...
        actions: [
          ...
          if (meeting['jitsiRoomId'] != null)
            ElevatedButton(
              onPressed: () {
                Navigator.of(context).pop();
                _joinSpecificMeeting(meeting);
              },
              style: ElevatedButton.styleFrom(
                backgroundColor: Colors.green,
                foregroundColor: Colors.white,
  ...
  void _startInstantMeeting() async {
    try {
      ...
      final userName = user?.displayName ?? 'SEWS User';
      final userEmail = user?.email ?? '';

      // Start instant meeting using CallService
      final roomId = await CallService.createMeeting(
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Instant meeting started: $roomId'),
          action: SnackBarAction(
            label: 'Share',
            onPressed: () {
              // Here you could add sharing functionality
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(content: Text('Meeting URL: https://meet.sewsconnect.com/$roomId')),
              );
            },
          ),
        ),
  ...
  void _createMeeting() async {
    ...
    try {
      ...
      if (meetingId != null) {
        ...
        final meetingTitle = _titleController.text;
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Meeting "$meetingTitle" created successfully!')),
          );
